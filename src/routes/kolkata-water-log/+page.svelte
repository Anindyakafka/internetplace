		...
		void import('maplibre-gl').then((ml) => {
			...
			map.on('load', () => {
				...
				map.addLayer({ id: 'water-accumulation', type: 'fill', source: 'water-model', paint: {
					'fill-color': ['match', ['get','VALUE'], 1, 'rgba(196,205,208,0.125)', 2, '#abced0', 3, '#519ea2', 'rgba(196,205,208,0.125)'],
				...
				if (context) {
					context.fillStyle = '#417d77'; context.strokeStyle = '#313e40'; context.lineWidth = 2;
					context.beginPath(); context.roundRect(3,3,122,50,25); context.fill(); context.stroke();
					map.addImage('report-pill', context.getImageData(0,0,128,56), { pixelRatio: 2, stretchX: [[28,100]], stretchY: [[20,36]], content: [22,10,106,46] });
				}
...
<main class="page">
	...
	<div class:hidden={open} class="water-slider">
		<span aria-hidden="true">≋</span><input aria-label="Water-layer opacity" type="range" min="0" max="100" bind:value={opacity} oninput={updateOpacity} style={`--p:${opacity}%`} />
	</div>
	...
	{#if notice}<div class="veil"><button class="dismiss" aria-label="Close location notice" onclick={() => notice = ''}></button><div class="alert" role="alertdialog" tabindex="-1"><span class="alert-art" aria-hidden="true"></span><h2>Oops!</h2><p>{notice}</p><button onclick={() => notice = ''}>Close</button></div></div>{/if}
</main>
