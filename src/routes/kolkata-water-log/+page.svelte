		...
		void import('maplibre-gl').then((ml) => {
			...
			map.on('load', () => {
				...
				for (const report of waterLogReports) {
					...
					markers.push(new ml.Marker({ element: el, anchor: 'left', offset: [0,-10], pitchAlignment: 'viewport', rotationAlignment: 'viewport', subpixelPositioning: false }).setLngLat(report.coordinates).addTo(map));
				}
			});
