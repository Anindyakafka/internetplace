<script lang="ts">
	...
	onMount(async () => {
		try {
			const response = await fetch('/data/sealdah-network.json');
...
<svelte:head>
	<title>Sealdah Local Lines — Anindya Singh</title>
	<meta name="description" content="All suburban train services beginning or ending at Sealdah, mapped over a historical map of Bengal." />
</svelte:head>

<main class="train-page">
	<header class="page-intro">
		<p class="eyebrow">Local lines · Sealdah network</p>
		<h1>Sealdah, in motion.</h1>
		<p class="lede">Every listed suburban service beginning or ending at Sealdah, traced across 29 terminal corridors on a historical map. Positions are calculated from timetable times and route distance, with reported data checked for the selected train.</p>
	...
	{:else}
		<section class="network-layout" aria-label="Sealdah train explorer">
			<aside class="train-sidebar">
				<div class="search-block"><label for="train-search">Find a train or destination</label><input id="train-search" type="search" bind:value={query} placeholder="31611, Ranaghat, RHA…" /></div>
				<div class="filters">
					<div class="direction-filter" aria-label="Direction filter">
						<button class:active={direction === 'all'} onclick={() => direction = 'all'}>All</button>
						<button class:active={direction === 'from-sealdah'} onclick={() => direction = 'from-sealdah'}>From Sealdah</button>
						<button class:active={direction === 'to-sealdah'} onclick={() => direction = 'to-sealdah'}>To Sealdah</button>
					</div>
					<select bind:value={terminal} aria-label="Terminal corridor"><option value="all">All 29 corridors</option>{#each terminals as item}<option value={item.code}>{item.name} ({item.serviceCount})</option>{/each}</select>
				</div>
