...
export default async () => {
	const key = process.env.DMRC_API_KEY;
	if (!key) return Response.json({ error: 'DMRC real-time data is not configured.' }, { status: 503 });
	...
	try {
		...
		if (!upstream.ok) return Response.json({ error: `DMRC returned HTTP ${upstream.status}.` }, { status: upstream.status });
		...
		const vehicles = (feed.entity ?? []).flatMap((entity) => {
			...
			return [{
				...
				odometer: vehicle.position.odometer ?? null,
		...
		return Response.json({ error: 'The DMRC vehicle feed could not be decoded.' }, { status: 502 });
	}
};
